...
<div class="text-xs"><div class="font-semibold text-white"><?=Security::e($u['nama']??'')?></div><div class="text-emerald-100"><?=Security::e($role)?> • v<?=Security::e($appVer)?></div></div></div></aside>
...
<?=$content??''?></main>
<footer class="px-6 pb-6 text-center text-[11px] text-emerald-800/60 no-print"><?=Security::e($appName)?> v<?=Security::e($appVer)?> • <?=date('Y')?> • Modern • Aman • Responsif</footer></div></div>
...
async function askUpdate(){const c=await Swal.fire({title:'Perbarui sistem ke versi terbaru?',html:'Data transaksi, upload, dan database <b>tidak diubah</b>.<br>Backup otomatis dibuat sebelum pembaruan.<br>Lanjut perbarui sekarang?',icon:'question',showCancelButton:true,cancelButtonText:'Batal',confirmButtonText:'Ya, Perbarui',confirmButtonColor:'#059669'});if(!c.isConfirmed)return;
let prog=8,timer=null;Swal.fire({title:'Memperbarui sistem...',html:'Mengambil versi terbaru dan memverifikasi berkas.<div class="mt-3 h-2.5 bg-emerald-100 rounded-full overflow-hidden"><div id="upBar" class="h-2.5 rounded-full bg-emerald-600 transition-all" style="width:8%"></div></div><div id="upTxt" class="mt-1 text-xs text-emerald-800 font-bold">8%</div><div id="upStep" class="mt-1 text-xs text-gray-500">Menyiapkan...</div>',allowOutsideClick:false,allowEscapeKey:false,showConfirmButton:false,didOpen:()=>{Swal.showLoading();timer=setInterval(()=>{prog=Math.min(92,prog+Math.random()*9+3);const e1=document.getElementById('upBar'),e2=document.getElementById('upTxt');if(e1)e1.style.width=prog+'%';if(e2)e2.innerText=Math.round(prog)+'%';},300);}});
let j=null,jerr='';try{const r=await fetch((window.__BASE||BASE)+'api/x',{method:'POST',body:new URLSearchParams({csrf_token:window.__CSRF||CSRF,act:'sys_update'})});j=await r.json();}catch(ex){jerr=ex.message;}
