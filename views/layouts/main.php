...
.dd-btn{width:100%;max-width:100%;display:flex;align-items:center;justify-content:space-between;gap:.5rem;border:1.5px solid #6ee7b7 !important;border-radius:.9rem !important;background:#fff !important;color:#064e3b !important;padding:.5rem .8rem;font-size:inherit;cursor:pointer;text-align:left;line-height:1.25;overflow:hidden}
...
function openModal(html){const m=document.getElementById('modal');if(!m)return;document.getElementById('modalBox').innerHTML=html;m.classList.remove('hidden');m.classList.add('flex');document.body.style.overflow='hidden';document.documentElement.style.overflow='hidden';m.scrollTop=0;if(typeof ddify==='function')setTimeout(()=>ddify(document.getElementById('modalBox')),0);}
...
<?php if(!empty($mlogo['logo'])&&is_file(__DIR__.'/../uploads/logo/'.$mlogo['logo'])):?><img src="<?=BASE_URL?>uploads/logo/<?=Security::e($mlogo['logo'])?>" class="w-11 h-11 rounded-2xl object-contain bg-white p-1 shadow-lg"><?php else:?><div class="w-11 h-11 rounded-2xl bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-xl shadow-lg"><i class="fa-solid fa-mosque"></i></div><?php endif;?>
...
<?php $mitems=[['bidang','Bidang','fa-layer-group'],['sumber_dana','Sumber Dana','fa-sack-dollar'],['jenis_belanja','Jenis Belanja','fa-tags'],['satuan','Satuan','fa-ruler'],['rekening','Rekening','fa-book'],['kegiatan','Kegiatan','fa-list-check'],['tahun_anggaran','Tahun Anggaran','fa-calendar']]; $mcur=($_GET['tab']??$arg??''); $mopen=($cur==='master')?'':''; ?>
<div>
...
<?php if(in_array($role,['superadmin','kepala_madrasah'])):?><a href="<?=BASE_URL?>audit" data-p="audit" class="navlink <?=($cur==='audit'?'active':'')?>"><i class="fa-solid fa-clock-rotate-left w-5 text-center"></i> Audit Log</a><?php endif;?>
...
<span class="hidden sm:flex items-center gap-2 bg-white/10 border border-white/20 rounded-full pl-1 pr-3 py-1"><span class="w-7 h-7 rounded-full bg-emerald-500 text-white flex items-center justify-center text-xs font-bold"><?=$initial?></span><span class="font-semibold text-white"><?=Security::e($u['nama']??'')?></span><span class="text-[10px] bg-emerald-500 text-white px-2 py-px rounded-full"><?=Security::e($role)?></span></span>
<button type="button" onclick="askLogout()" class="w-9 h-9 rounded-xl bg-red-500/20 border border-red-400/40 text-red-200 flex items-center justify-center hover:bg-red-500/30" title="Logout"><i class="fa-solid fa-right-from-bracket"></i></button></div></header>
...
<?=$content??''?></main>
<footer class="px-6 pb-6 text-center text-[11px] text-emerald-800/60 no-print">RKAM Madrasah v<?=APP_VERSION?> • Modern • Aman • Responsif</footer></div></div>
