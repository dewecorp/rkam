...
<input name="penanggung_jawab" placeholder="Penanggung Jawab" value="${d.penanggung_jawab||''}" class="border rounded p-2">
<input name="waktu_pelaksanaan" placeholder="Waktu Pelaksanaan" value="${d.waktu_pelaksanaan||''}" class="border rounded p-2">
<select name="prioritas" class="border rounded p-2">${['rendah','sedang','tinggi','mendesak'].map(p=>`<option ${d.prioritas===p?'selected':''}>${p}</option>`).join('')}</select></div>
<textarea name="tujuan" placeholder="Tujuan" class="w-full border rounded p-2">${d.tujuan||''}</textarea>
...
openModal(h);renderItems(d.items&&d.items.length?d.items:[{uraian:'',volume:1,satuan:'',harga_satuan:0}]);renderSums(d.sumber&&d.sumber.length?d.sumber:[]);}
...
function itemRow(it){it=it||{};const cur=it.satuan||it.satuan_text||'';const inList=SAT.some(s=>s.nama_satuan.toLowerCase()===(cur||'').toString().toLowerCase());return `<div class="grid grid-cols-12 gap-1 irow bg-gray-50 p-2 rounded-xl border border-emerald-100">
<input class="col-span-4 border rounded-xl p-1.5 iu" placeholder="Uraian *" value="${(it.uraian||'').replaceAll('"','')}">
<input type="number" step="0.01" class="col-span-2 border rounded-xl p-1.5 iv" value="${it.volume??1}" oninput="calc()">
<select class="col-span-2 border rounded-xl p-1.5 is bg-white">${satOpts(inList?cur:'')}${(!inList&&cur)?`<option value="${cur.replaceAll('"','')}" selected>${cur.replaceAll('<','&lt;')}</option>`:''}</select>
<input type="number" step="0.01" class="col-span-3 border rounded-xl p-1.5 ih" value="${it.harga_satuan??0}" oninput="calc()">
<button type="button" onclick="this.parentElement.remove();calc()" class="col-span-1 text-red-600">x</button>
...
function renderSums(a){const opts=SUM.map(s=>({id:s.id,nama:s.nama_sumber_dana}));document.getElementById('sums').innerHTML=(a||[]).map(s=>{const cur=s.sumber_dana_id||s.id||'';return `<div class="flex gap-2 srow items-center bg-gray-50 p-2 rounded-xl border border-emerald-100"><select class="sid border rounded-xl p-1.5 flex-1 bg-white">${opts.map(o=>`<option value="${o.id}" ${String(cur)===String(o.id)?'selected':''}>${o.nama}</option>`).join('')}</select><input type="number" min="0" step="0.01" class="border rounded-xl p-1.5 sj w-40" value="${s.jumlah||0}" oninput="calcSum()"><button type="button" onclick="this.parentElement.remove();calcSum()" class="text-red-600 px-1">x</button></div>`;}).join('');if(!a||!a.length)addSumberRow();calcSum();if(typeof ddify==='function')setTimeout(()=>ddify(document.getElementById('modalBox')),0);}
function addSumberRow(){const opts=SUM.map(s=>`<option value="${s.id}">${s.nama_sumber_dana}</option>`).join('');document.getElementById('sums').insertAdjacentHTML('beforeend',`<div class="flex gap-2 srow items-center bg-gray-50 p-2 rounded-xl border border-emerald-100"><select class="sid border rounded-xl p-1.5 flex-1 bg-white">${opts}</select><input type="number" min="0" step="0.01" class="border rounded-xl p-1.5 sj w-40" value="0" oninput="calcSum()"><button type="button" onclick="this.parentElement.remove();calcSum()" class="text-red-600 px-1">x</button></div>`);calcSum();if(typeof ddify==='function')setTimeout(()=>ddify(document.getElementById('modalBox')),0);}
function addSumber(){addSumberRow();}
function getItems(){return [...document.querySelectorAll('.irow')].map(r=>({uraian:r.querySelector('.iu').value,volume:parseFloat(r.querySelector('.iv').value)||0,satuan:r.querySelector('.is').value,harga_satuan:parseFloat(r.querySelector('.ih').value)||0}));}
function getSums(){return [...document.querySelectorAll('.srow')].map(r=>({id:r.querySelector('.sid').value,jumlah:parseFloat(r.querySelector('.sj').value)||0}));}
