...
?>
<div class="bg-white rounded shadow overflow-auto"><table class="w-full text-sm min-w-[800px]"><thead><tr class="bg-gray-50 border-b text-left"><th class="p-2 w-10">No</th><th class="p-2">Kegiatan</th><th class="p-2 text-right">Anggaran</th><th class="p-2 text-right">Realisasi</th><th class="p-2 text-right">Sisa</th><th class="p-2 w-48">Serapan</th><th class="p-2">Status</th></tr></thead><tbody>
<?php $no=1;foreach($rows as $r):$sisa=(float)$r['total_anggaran']-(float)$r['jml_real'];$p=Helper::persen($r['jml_real'],$r['total_anggaran']);?>
<tr class="border-b"><td class="p-2"><?=$no++?></td><td class="p-2"><a href="<?=BASE_URL?>rkam/detail/<?=$r['id']?>" class="font-semibold text-emerald-800"><?=Security::e($r['nama_kegiatan'])?></a><div class="text-xs text-gray-500"><?=Security::e($r['nama_bidang']??'')?></div></td>
...
<?php endforeach;?><?php if(!$rows):?><tr><td colspan="7" class="p-8 text-center text-gray-500">Belum ada data.</td></tr><?php endif;?></tbody></table></div>
